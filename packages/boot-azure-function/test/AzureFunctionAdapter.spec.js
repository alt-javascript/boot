...
import { createAzureFunctionHandler, AzureFunctionAdapter } from '../index.js';
...
function azureRequest(method, url, overrides = {}) {
  return { method, url, params: {}, query: {}, headers: {}, body: null, ...overrides };
}
...
async function buildAdapter(extras = [], configOverrides = {}) {
  ...
  return new AzureFunctionAdapter(appCtx);
}

describe('Azure Function Adapter', () => {
  describe('route dispatch', () => {
    it('GET route returns service data', async () => {
      const adapter = await buildAdapter();
      const res = await adapter.handle(azureRequest('GET', '/greet/Alice'));
      assert.equal(res.status, 200);
      assert.deepEqual(res.jsonBody, { message: 'Hello, Alice!' });
    });

    it('POST route with JSON body', async () => {
      const adapter = await buildAdapter();
      const res = await adapter.handle(azureRequest('POST', '/echo', {
        body: { key: 'value' },
      }));
      assert.equal(res.status, 200);
      assert.deepEqual(res.jsonBody, { received: { key: 'value' } });
    });

    it('health endpoint', async () => {
      const adapter = await buildAdapter();
      const res = await adapter.handle(azureRequest('GET', '/health'));
      assert.equal(res.status, 200);
    ...
    it('returns 404 for unregistered routes', async () => {
      const adapter = await buildAdapter();
      const res = await adapter.handle(azureRequest('GET', '/nonexistent'));
      assert.equal(res.status, 404);
    });
  });

  describe('createAzureFunctionHandler', () => {
    it('boots CDI and handles requests', async () => {
      ...
      const res = await handler(azureRequest('GET', '/greet/Azure'), {});
      assert.equal(res.status, 200);
      assert.deepEqual(res.jsonBody, { message: 'Hello, Azure!' });
    });
    ...
    it('reuses context on subsequent requests', async () => {
      ...
      const res1 = await handler(azureRequest('GET', '/health'), {});
      assert.equal(res1.status, 200);
      const res2 = await handler(azureRequest('GET', '/health'), {});
  ...
  describe('DI wiring', () => {
    it('autowires service into controller', async () => {
      const adapter = await buildAdapter();
      const ctrl = adapter.applicationContext.get('greetingController');
      assert.instanceOf(ctrl.greetingService, GreetingService);
    });
  });

  describe('JSDBC integration', () => {
    it('full stack: Azure Function → controller → service → JSDBC', async () => {
      const adapter = await buildAdapter(
        [
      ...
      let res = await adapter.handle(azureRequest('GET', '/todos'));
      assert.equal(res.status, 200);
      assert.deepEqual(res.jsonBody, []);

      res = await adapter.handle(azureRequest('POST', '/todos', {
        body: { title: 'Buy milk' },
      }));
      assert.equal(res.status, 201);
      assert.equal(res.jsonBody.title, 'Buy milk');

      res = await adapter.handle(azureRequest('GET', '/todos'));
