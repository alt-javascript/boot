/**
 * server.js — local dev server for the Vue CDN example
 *
 * Serves:
 *   /             → index.html (CDN import map)
 *   /dev          → dev.html (local importmap, no CDN required)
 *   /src/*        → local source files
 *   /node_modules/* → package files for local import map
 *
 * Run: npm run serve
 * Open: http://localhost:3000/dev  (local packages, no internet needed)
 *        http://localhost:3000/    (CDN imports)
 */
