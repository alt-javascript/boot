...
describe('CachingLoggerFactory Specification', () => {
  ...
  it('static getLogger uses explicit config when provided,', () => {
    const explicitConfig = new EphemeralConfig({ logging: { level: { ROOT: 'info' } } });
    const logger = CachingLoggerFactory.getLogger(Logger.DEFAULT_CATEGORY, explicitConfig);

    assert.equal(logger.config, explicitConfig, 'logger.config === explicitConfig');
    assert.equal(logger.category, Logger.DEFAULT_CATEGORY, 'logger.category === Logger.DEFAULT_CATEGORY');
    assert.equal(logger.configPath, ConfigurableLogger.DEFAULT_CONFIG_PATH, 'logger.configPath === ConfigurableLogger.DEFAULT_CONFIG_PATH');
  });
